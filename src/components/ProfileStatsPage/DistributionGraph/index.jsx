...
export default class DistributionGraph extends React.Component {
  ...
  componentDidMount() {
    ...
    this.resizeHandle = () => {
      if (
        ...
        ($scope.desktop ||
          this.mobileWidth !==
            DistributionGraph.getMobileWidthGrapthMeasurements())
      ) {
        $scope.desktop = false;
        this.draw();
  ...
  draw() {
    ...
    const xScale2 = d3.scale
      .linear()
      .domain([ranges[0].start, d3.max(ranges, (range) => range.end)])
    ...
    const xAxis = d3.svg
      ...
      .ticks(ranges.length);
    const yAxis = (ticks) =>
      d3.svg.axis().scale(yScale).orient("left").ticks(ticks);
    const svg = d3
    ...
    svg
      ...
      .append("line")
      .attr("class", "xaxis")
      .attr("x1", (d, i) => (i === 0 ? padding.left : xScale(i) - 0.5))
      .attr("x2", (d, i) =>
        i === ranges.length - 1
