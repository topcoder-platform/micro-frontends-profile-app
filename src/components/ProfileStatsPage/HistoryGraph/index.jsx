...
export default class HistoryGraph extends React.Component {
  ...
  draw() {
    ...
    history.sort(
      ({ ratingDate: d1 }, { ratingDate: d2 }) => moment(d1) - moment(d2)
    );
    ...
    if (_.isEmpty(history)) {
      y.domain([1000, 1600]);
      const fromDate = d3.time.format
        .utc("%Y-%m-%dT%H:%M:%SZ")
        .parse(moment().utc().subtract(2, "years").format());
      const toDate = d3.time.format
        .utc("%Y-%m-%dT%H:%M:%SZ")
        .parse(moment().utc().format());
    ...
    svg
      .selectAll(`g.${styles.x}.${styles.axis} .tick text`)
      .attr("font-weight", (d) =>
        moment(d).format("MM") === "01" ? "bold" : "normal"
      )
      .attr("fill", (d) =>
        moment(d).format("MM") === "01" ? "black" : "#a3a3ad"
      )
      .attr("font-size", () => 11);
    ...
    function getChallengeLink(challengeId) {
      ...
      if (track === "DATA_SCIENCE") {
        if (subTrack === "MARATHON_MATCH") {
          return `${URL.COMMUNITY}/tc?module=MatchDetails&rd=${challengeId}`;
        }
        if (subTrack === "SRM") {
          return `${URL.COMMUNITY}/stat?c=round_overview&rd=${challengeId}`;
        }
      }
      return null;
    }
    ...
    svg
      ...
      .attr("y1", (d, i) =>
        processRatingStripePoint(y(i === 0 ? 0 : RATING_COLORS[i - 1].limit))
      )
